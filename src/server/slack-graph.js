export async function buildSlackGraph({ token, historyLimit, slackApi }) {
  ...
  const edgeList = Array.from(edges.values())
    ...
    .map((edge) => ({
      ...
      relationship: classifyRelationship(edge, personSignals.get(otherPersonIdForEdge(edge, me.id))),
      topTopics: extractTopTopics(personSignals.get(otherPersonIdForEdge(edge, me.id)), 5)
    }));
...
function extractTopTopics(signal, limit) {
  ...
  return Array.from(signal.topicScores.entries())
    .sort((a, b) => b[1] - a[1])
    .slice(0, limit)
...
function ingestMessageText(signal, text) {
  const tokens = tokenizeSlackText(text);
  if (tokens.length === 0) {
    return;
  }
